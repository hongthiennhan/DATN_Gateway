...
thread_pause_t udp_pause = {.is_paused = false,
...
static int udp_connect(const char *server_url) {
  ...
  if (server_url && strlen(server_url) > 0) {
    ...
    snprintf(url, sizeof(url), "udp://%s:%d", config->server_host,
             config->server_port);
  }
...
#ifdef DEBUG
  printf("UDP: %s to %s\n", udp_connection ? "Connecting" : "Failed to connect",
         url);
#endif
...
void udp_update_received_data(const unsigned char *data, uint16_t data_len) {
  ...
  size_t copy_len = data_len < sizeof(udp_last_received_data) - 1
                        ? data_len
                        : sizeof(udp_last_received_data) - 1;
...
static void udp_update_downlink_data(unsigned char *downlink_buffer,
                                     size_t buffer_size,
    ...
    if (config && config->features.status_reporting) {
      ...
      if (sys_info) {
        ...
        snprintf(handshake, sizeof(handshake),
                 ...
                 udp_get_local_ip(), sys_info->firmware_version,
...
static void build_udp_telemetry_payload(char *payload, size_t payload_size,
                                        time_t timestamp) {
  ...
  if (strlen(udp_last_received_data) > 0) {
    ...
    escape_json_string(udp_last_received_data, escaped_data,
                       sizeof(escaped_data));
...
void *udp_thread_func(void *arg) {
  ...
  char server_url[256];
  snprintf(server_url, sizeof(server_url), "udp://%s:%d", config->server_host,
           config->server_port);
  ...
  while (1) {
    ...
    if (udp_connected && current_time - last_publish >= config->send_interval) {
      build_udp_telemetry_payload(telemetry_payload,
