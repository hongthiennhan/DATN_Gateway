...
unsigned char *UART_Read_Response(uint32_t timeout_ms,
  ...
  *bytes_read_out = 0;
  ...
  int buffer_size =
      config ? config->response_buffer_size : 2048; // fallback to 2048
  ...
  uint32_t poll_interval =
      config ? config->poll_interval_ms : 20; // fallback to 20ms
...
speed_t UART_map_to_speed(uint32_t baud_num) {
...
#endif
        if (strcmp(speed_code, "B9600") == 0)
          return B9600;
        if (strcmp(speed_code, "B19200") == 0)
          return B19200;
        if (strcmp(speed_code, "B38400") == 0)
          return B38400;
        if (strcmp(speed_code, "B57600") == 0)
          return B57600;
        if (strcmp(speed_code, "B115200") == 0)
          return B115200;
        if (strcmp(speed_code, "B230400") == 0)
          return B230400;
        if (strcmp(speed_code, "B460800") == 0)
          return B460800;
        if (strcmp(speed_code, "B921600") == 0)
          return B921600;
      }
    }
  }
  ...
  if (error_msg) {
    snprintf(error_msg, error_buf_size,
             "Unsupported baudrate: %u. Using default %d.\n", baud_num,
             fallback_rate);
