...
\section{Hướng phát triển trong tương lai}
    Mặc dù sản phẩm đã đạt được những kết quả khả quan và đáp ứng đầy đủ các mục tiêu thiết kế cốt lõi của một IoT Gateway dạng mô-đun, việc đưa thiết bị vào vận hành thực tế trong môi trường công nghiệp đòi hỏi cần đạt được những tiêu chuẩn khắt khe. Trong tương lai, sản phẩm cần được đánh giá dựa trên các tiêu chuẩn công nghiệp nhằm đạt được những yêu cầu thực tế, mục tiêu có thể đưa sản phẩm ra thị trường.
    ...
    \begin{itemize}
        ...
        \item \textbf{Tối ưu hóa pipeline dữ liệu bằng kiến trúc hướng sự kiện (Event-driven):} Kết quả phân tích hiệu suất tại Mục 4.6 chỉ ra rằng dù bus SPI chỉ chiếm 6,93\% băng thông trong điều kiện xấu nhất, rủi ro tràn bộ đệm (buffer overflow) vẫn tồn tại khi LAN-MCU phải phục vụ các ngắt ngoại vi liên tục. Do đó, việc chuyển đổi sang kiến trúc hướng sự kiện kết hợp với cơ chế truy cập bộ nhớ trực tiếp (DMA) là lựa chọn kỹ thuật tất yếu để giải phóng tài nguyên CPU, đảm bảo duy trì tỷ lệ truyền gói tin thành công (PDR) $>$ 85\% ngay cả dưới tải cực nặng.
        ...
        \item \textbf{Xây dựng cơ chế tự phục hồi (Self-healing) dựa trên State Machine:} Hệ thống cần bổ sung các máy trạng thái giám sát chuyên sâu để tự động phát hiện và khôi phục các lỗi logic ngoại vi (như treo bus I2C hoặc mất đồng bộ module adapter). Việc tự động hóa quy trình xử lý lỗi này sẽ đảm bảo tính sẵn sàng cao của thiết bị mà không cần sự can thiệp vật lý tại hiện trường.
    \end{itemize}
