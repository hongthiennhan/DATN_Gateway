...
\subsection{Kiến trúc hệ thống và chuỗi truyền dẫn dữ liệu}
        ...
        \end{figure}
        % ---------------------------------------------------------------
        \subsection{Thông lượng dữ liệu và hiệu suất băng thông ($\eta$)}
...
Nút thắt thực tế không phải là air interface mà là giao tiếp UART từ module E18-ZG120 lên LAN-MCU. Module đóng gói mỗi khung Zigbee (127 byte PSDU) vào khung HEX của Ebyte với 6--7 byte framing overhead, tạo ra frame UART $\approx 133$--$134$ byte. Tại tốc độ $115200\,\text{bps}$ với 10 bit/byte (8N1), thời gian truyền một frame là:
\[ T_{UART,Zigbee} = \frac{134 \times 10}{115200} \approx 11{,}6\,\text{ms} \]
Điều này khiến UART chậm hơn air interface $11{,}6\,\text{ms} / 5{,}25\,\text{ms} \approx 2{,}2\times$, và trở thành điểm nghẽn của toàn chặng Zigbee.
...
\begin{longtable}{|p{3.2cm}|p{2.3cm}|p{2.7cm}|p{1.4cm}|p{4.2cm}|}
    ...
    \endfoot
    \endlastfoot
    Zigbee & $250\,\text{kbps}$ & $\approx 8{,}2\,\text{kbps}$ & $3{,}3\%$ & CSMA/CA contention \& UART 11,6\,ms/frame \\ \hline
    LoRa SF7/125 (P2P) & $5{,}47\,\text{kbps}$ & $\approx 3{,}5\,\text{kbps}$ & $64\%$ & Time-on-Air ($\approx 97{,}5\,\text{ms/gói}$) \\ \hline
    BLE (không DLE) & $1000\,\text{kbps}$ & $\approx 8\,\text{kbps}$ & $0{,}8\%$ & MTU = 20 byte, $CI = 20\,\text{ms}$ \\ \hline
    BLE (bật DLE) & $1000\,\text{kbps}$ & $\approx 97{,}6\,\text{kbps}$ & $9{,}8\%$ & $CI = 20\,\text{ms}$ \\ \hline
\end{longtable}
...
\begin{itemize}
    ...
    \item \textbf{$T_{UART}$ -- Độ trễ giao tiếp module--MCU:} Zigbee $\approx 11{,}6\,\text{ms}$ ($\approx 134$ byte Ebyte frame tại 115200 bps, 8N1), LoRa $\approx 16{,}1\,\text{ms}$ (AT command + phản hồi TX DONE). BLE không có thành phần này do tích hợp native trên LAN-MCU.
    \item \textbf{$T_{CI}$ -- Độ trễ chờ Connection Interval (chỉ BLE):} Vì BLE chỉ truyền dữ liệu tại các thời điểm cố định cách nhau $CI = 20\,\text{ms}$, một gói đến bất kỳ thời điểm nào trong chu kỳ phải chờ đến interval tiếp theo. Độ trễ chờ này dao động $0$--$CI = 0$--$20\,\text{ms}$, trung bình $CI/2 = 10\,\text{ms}$. Zigbee và LoRa không có thành phần này.
    \item \textbf{$T_{batch}$ -- Độ trễ gom gói:} Dao động $0$--$10\,\text{ms}$ tùy thời điểm dữ liệu đến trong chu kỳ batch flush của LAN-MCU.
    \item \textbf{$T_{SPI}$ -- Độ trễ bus Inter-MCU:} Tại xung nhịp 60\,MHz với DMA, thời gian truyền trong điều kiện tải trung bình thực tế ($\approx 2393$ byte) là $\approx 0{,}32\,\text{ms}$; worst-case burst (5193 byte, xem mục~\ref{subsec:spi}) mất $\approx 0{,}693\,\text{ms}$.
...
Với BLE, độ trễ worst-case cần tính thêm $T_{CI,max} = 20\,\text{ms}$ (gói đến ngay sau khi vừa bỏ lỡ một interval):
\[ T_{e2e,BLE,worst} = T_{air} + T_{CI,max} + T_{batch} + T_{SPI} + T_{JSON} + T_{network} \approx 2{,}2 + 20 + 10 + 0{,}32 + 0{,}38 + 30 \approx 62{,}9\,\text{ms} \]
...
\begin{longtable}{|p{4.5cm}|p{4.5cm}|p{5cm}|}
    ...
    \endfoot
    \endlastfoot
    Zigbee $\rightarrow$ MQTT & $47{,}6\,\text{ms}$--$57{,}6\,\text{ms}$ & $T_{UART}$ (11,6 ms) \& $T_{network}$ (30 ms) \\ \hline
    LoRa SF7 (P2P) $\rightarrow$ MQTT & $144{,}3\,\text{ms}$--$154{,}3\,\text{ms}$ & $T_{air}$ (97,5 ms) -- chiếm $\approx 65\%$ \\ \hline
    BLE $\rightarrow$ MQTT (trung bình) & $42{,}9\,\text{ms}$--$52{,}9\,\text{ms}$ & $T_{network}$ (30 ms) -- chiếm $\approx 65\%$ \\ \hline
...
\begin{longtable}{|p{3cm}|p{2cm}|p{3.5cm}|p{4.5cm}|}
    ...
    \endfoot
    \endlastfoot
    Zigbee   & 20 & 134 byte (PSDU + Ebyte overhead) & $20 \times 134 = 2680\,\text{byte}$ \\ \hline
    BLE (DLE) & 8  & 263 byte (LL PDU tối đa)         & $8 \times 263 = 2104\,\text{byte}$  \\ \hline
    LoRa P2P & 1  & 169 byte (AT command limit)       & $1 \times 169 = 169\,\text{byte}$   \\ \hline
    \textbf{Tổng} & \textbf{29} & -- & $\mathbf{4953\,\text{byte}}$ \\ \hline
\end{longtable}

Thời gian SPI cần thiết và hệ số sử dụng trong điều kiện worst-case:
\[ T_{SPI,worst} = \frac{4953 \times 8}{60 \times 10^6} \approx 0{,}661\,\text{ms} \]
\[ U_{SPI,worst} = \frac{0{,}661}{10} \approx 6{,}61\% \]
...
\textbf{Kết luận:} Bus SPI hoàn toàn không phải nút thắt -- hệ số sử dụng trung bình chỉ đạt $0{,}183\%$, worst-case $6{,}61\%$ (khi toàn bộ 29 node gửi frame tối đa đồng thời). Nguy cơ suy giảm PDR xuất phát từ Zigbee Listener buffer ($512\,\text{byte}$) bị tràn khi LAN-MCU bận phục vụ ngắt SPI/UART tần suất cao (mỗi $10\,\text{ms}$); đây là điểm cần tối ưu để đạt mục tiêu PDR toàn hệ thống $> 85\%$ dưới tải nặng.

\subsection{So sánh hiệu năng với Gateway tham chiếu M5Stack CoreS3}
\label{subsec:perf_compare}

Để định vị giá trị thực tiễn của kiến trúc Dual-MCU đề xuất, mục này đối chiếu sản phẩm đồ án với một giải pháp thương mại tiêu biểu là \textbf{M5Stack CoreS3} -- một nền tảng Single-MCU sử dụng một ESP32-S3 duy nhất gánh đồng thời toàn bộ tác vụ LAN (Zigbee, BLE, LoRa) và WAN (Wi-Fi, MQTT, TLS). Cấu hình so sánh giữ nguyên kịch bản tải đã sử dụng ở các mục trước: 20 node Zigbee, 8 node BLE và 1 node LoRa. Hai chỉ số phân biệt cốt lõi giữa hai kiến trúc là \emph{độ trễ End-to-End dưới tải} và \emph{khả năng chịu burst đồng thời}; thông lượng peak gần như không khác biệt do hai kiến trúc cùng bị giới hạn bởi UART 115200\,bps tới module Zigbee ($T_{UART} \approx 11{,}6\,\text{ms/frame}$), LoRa ($T_{UART} \approx 16{,}1\,\text{ms/frame}$) và Connection Interval của BLE.

\textbf{Tương đồng về thông lượng peak.} Cả hai kiến trúc đều đạt $R_{peak,Zigbee} \approx 62\,\text{kbps}$, $R_{peak,BLE} \approx 781\,\text{kbps}$ (8 thiết bị) và $R_{peak,LoRa} \approx 3{,}52\,\text{kbps}$ vì các bottleneck nằm ở giao thức và bus tới module ngoài, không phải ở MCU. Ưu thế thực sự của Dual-MCU không thể hiện ở thông lượng đỉnh mà ở \emph{tính ổn định khi tải tăng}.

\textbf{Khác biệt về độ trễ E2E theo mức tải.} Ở tải nhẹ, Single-MCU CoreS3 nhanh hơn nhẹ ($\approx 5\,\text{ms}$) do không phải chịu chu kỳ batch cố định 10\,ms của bus SPI. Tuy nhiên, dưới tải nặng (tất cả node phát đồng thời), CoreS3 phải chia sẻ một CPU duy nhất giữa nhận UART, BLE controller, cJSON, mbedTLS, Wi-Fi và MQTT, đồng thời chịu hiện tượng coexist Wi-Fi/BLE TDM ở băng 2,4\,GHz; hệ quả là $T_{internal}$ tăng từ $\approx 5\,\text{ms}$ lên $20$--$40\,\text{ms}$. Kiến trúc Dual-MCU tách biệt hoàn toàn LAN/WAN nên $T_{internal}$ giữ giá trị cố định $\approx 10{,}3\,\text{ms}$ không phụ thuộc tải. Bảng~\ref{tab:e2e_compare} tổng hợp kết quả ở cả hai chế độ tải; khoảng chênh lệch giữa CoreS3 tải nặng so với Dual-MCU là $\approx 19{,}6\,\text{ms}$ cho mọi giao thức, phản ánh đúng mức tăng $T_{internal}$ từ $5\,\text{ms}$ lên $30\,\text{ms}$.
...
\begin{longtable}{|p{4.2cm}|p{2.7cm}|p{3cm}|p{3cm}|}
    ...
    \endfoot
    \endlastfoot
    Zigbee $\rightarrow$ MQTT & $\approx 57{,}6\,\text{ms}$ & $\approx 52{,}2\,\text{ms}$ & $\approx 77{,}2\,\text{ms}$ \\ \hline
    BLE $\rightarrow$ MQTT & $\approx 42{,}9\,\text{ms}$ & $\approx 37{,}6\,\text{ms}$ & $\approx 62{,}6\,\text{ms}$ \\ \hline
    LoRa $\rightarrow$ MQTT & $\approx 154{,}3\,\text{ms}$ & $\approx 149{,}0\,\text{ms}$ & $\approx 174{,}0\,\text{ms}$ \\ \hline
...
\textbf{Nguồn gốc của ưu thế Dual-MCU.} Việc tách LAN-MCU và WAN-MCU thành hai miền xử lý độc lập mang lại ba lợi ích then chốt: (i) loại bỏ hoàn toàn coexist Wi-Fi/BLE 2,4\,GHz trên cùng một chip, (ii) cố định thời gian xử lý nội bộ ở mức deterministic 10,3\,ms thông qua chu kỳ batch SPI, và (iii) cho phép xử lý song song giữa nhận dữ liệu LAN và publish MQTT WAN. Đánh đổi là khoảng $5\,\text{ms}$ overhead ở tải nhẹ và độ phức tạp firmware cao hơn -- cả hai đều chấp nhận được trong các ứng dụng gateway IoT thực tế nơi tính ổn định và khả năng chịu tải quan trọng hơn độ trễ tối thiểu lúc nhàn rỗi.
...
\begin{longtable}{|p{6cm}|c|p{6cm}|}
    \caption{Bảng so sánh chi phí phần cứng.} \label{tab:cost_compare} \\
    \hline
    ...
    Baseboard Dual-MCU (gia công 1000 board) & $\mathdollar$19{,}3 & 2$\times$ ESP32-S3 + nguồn + bus SPI + I/O \\ \hline
    LoRaWAN adapter + Wio-E5 module & $\mathdollar$8{,}9 & STM32WLE5JC, AT command \\ \hline
    Zigbee adapter + E18-ZG120 module & $\mathdollar$7{,}6 & EFR32MG1B / CC2530, UART HEX \\ \hline
    Màn hình LCD 2.4'' cảm ứng HMI UART & $\mathdollar$2{,}3 & Hiển thị trạng thái + cấu hình \\ \hline
    Pin Li-Po 4000\,mAh & $\mathdollar$3{,}0 & Backup khi mất nguồn AC \\ \hline
    \textbf{Tổng cộng (Dual-MCU)} & \textbf{$\mathdollar$41{,}1} & \\ \hline
    \multicolumn{3}{|l|}{\textbf{Cấu hình tham chiếu (M5Stack CoreS3)}} \\ \hline
    M5Stack CoreS3 (kèm pin Li-Po 500\,mAh) & $\mathdollar$59{,}9 & ESP32-S3 + LCD + camera + mic + speaker + IMU + compass \\ \hline
    M5Stack COM.Zigbee (CC2630F128) & $\mathdollar$26{,}9 & Module Zigbee chính hãng \\ \hline
    M5Stack COM.LoRaWAN EU868 (STM32WLE5) & $\mathdollar$24{,}9 & Module LoRaWAN chính hãng \\ \hline
    \textbf{Tổng cộng (CoreS3)} & \textbf{$\mathdollar$111{,}7} & \\ \hline
\end{longtable}

Tổng chi phí phần cứng của sản phẩm đồ án thấp hơn cấu hình M5Stack CoreS3 khoảng \textbf{$\mathdollar$70{,}6}, tương đương mức giảm \textbf{$\approx 63\%$}. Lợi thế giá thành đến từ việc thiết kế phần cứng theo đúng yêu cầu chức năng của một Gateway IoT, loại bỏ hoàn toàn các thành phần dư thừa của một nền tảng phát triển đa năng như CoreS3 -- bao gồm camera, microphone, loa, IMU và compass BMM150. Những thành phần này không phục vụ vai trò Gateway (nhận dữ liệu từ node LAN, gom batch và đẩy lên Cloud) nhưng vẫn chiếm tỉ trọng đáng kể trong giá bán của CoreS3.

Đồng thời, dung lượng pin Li-Po của sản phẩm đồ án (4000\,mAh) lớn hơn $8\times$ so với pin tích hợp trong CoreS3 (500\,mAh), cho phép Gateway tiếp tục vận hành khi mất nguồn AC trong thời gian đủ dài để hệ thống IoT duy trì kết nối tới Cloud -- một yêu cầu thiết yếu mà CoreS3 không đáp ứng được nếu không bổ sung module pin riêng.

Khi kết hợp với phân tích hiệu năng ở mục~\ref{subsec:perf_compare}, có thể kết luận rằng sản phẩm đồ án đạt \textbf{tính ổn định và khả năng chịu tải tốt hơn} (latency deterministic, jitter $\pm 5$--$10\,\text{ms}$, burst nhanh hơn $46\%$) ở \textbf{mức chi phí thấp hơn $63\%$}. Sự đánh đổi duy nhất là độ trễ ở tải nhẹ tăng nhẹ ($\approx 5\,\text{ms}$) -- không ảnh hưởng đến các ứng dụng telemetry IoT điển hình. Đây là minh chứng cho tính khả thi thương mại của thiết kế Dual-MCU đối với phân khúc Gateway IoT chuyên dụng.