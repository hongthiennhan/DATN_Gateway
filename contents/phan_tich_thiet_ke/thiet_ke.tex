\section{Thiết kế}
    \subsection{Thiết kế Kiến trúc Hệ thống (System Architecture Design)}
        ...
        \subsubsection{Mô tả thành phần hệ thống}
            ...
            \paragraph{Khối Module Adapter}\par
                Đây là thành phần cho phép tháo lắp và thay thế linh hoạt các mô-đun, được chuẩn hóa kết nối với Baseboard gồm các giao thức: UART, SPI, I2C, USB, và các GPIO.
                \begin{itemize}
                    \item \textbf{WAN Module Adapter}
                    \begin{itemize}
                        \item Kết nối với Master WAN-MCU.
                        \item Hỗ trợ các mô-đun giao tiếp với Cloud hoặc thiết bị người dùng (User Interface Devices).
                    \end{itemize}
                    \item \textbf{LAN1 Module Adapter}
                    \begin{itemize}
                        \item Kết nối với Slave LAN-MCU.
                        \item Hỗ trợ kết nối các mô-đun kết nối không dây như LoRa, Zigbee, BLE, Thread, \dots
                    \end{itemize}
                    \item \textbf{LAN2 Module Adapter}
                    \begin{itemize}
                        \item Kết nối với Slave LAN-MCU.
                        \item Hỗ trợ kết nối các mô-đun kết nối có dây như RS-485, CAN, \dots
                    \end{itemize}
                \end{itemize}
            ...
            \paragraph{Configuration Tools:}\par
                Một chương trình máy tính để cấu hình cho gateway thực hiện những chức năng cho các mô-đun được kết nối.
        ...
        \subsubsection{Thiết kế Module hóa}
            \paragraph{Nguyên lý:}\par
                Thiết kế mô-đun hóa cho phép người dùng dễ dàng thay thế, nâng cấp hoặc mở rộng các chức năng của gateway mà không cần thay đổi toàn bộ hệ thống. Mỗi mô-đun được thiết kế theo chuẩn kết nối chung, giúp đảm bảo tính tương thích và linh hoạt trong việc lựa chọn các mô-đun phù hợp với nhu cầu sử dụng cụ thể.
            \paragraph{Thiết kế:}\par
                Đối với các mô-đun sẵn có trên thị trường, giao diện kết nối thường không tuân theo một tiêu chuẩn thống nhất, kể cả với các mô-đun sử dụng chuẩn cơ khí như Mini PCIe hay M.2. Vì vậy, việc thiết kế các Module Adapter là cần thiết để chuẩn hóa kết nối giữa mô-đun và baseboard. Trong thiết kế này, cổng chuẩn vật lý PCI Express x1 (36-pin Card Edge Connector) được lựa chọn làm giao diện kết nối giữa Module Adapter và baseboard. Tuy nhiên, sơ đồ chân trên cổng được định nghĩa lại để phù hợp với kiến trúc phần cứng của hệ thống.
        ...
        \subsubsection{Xử lý dữ liệu}
            ...
            \paragraph{Luồng Uplink: Sensor $\to$ LAN-MCU $\to$ WAN-MCU $\to$ Cloud}
                ...
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.65\textwidth]{figures/DATN/sequence_diagram_end-to-end_uplink.png}
            ...
            \paragraph{Luồng Downlink: Cloud $\to$ WAN-MCU $\to$ LAN-MCU $\to$ Sensor}
                ...
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.65\textwidth]{figures/DATN/sequence_diagram_end-to-end_downlink.png}
            ...
            % \paragraph{Cơ chế cập nhật cấu hình cho Gateway từ App Config PC}
                ...
                % \begin{itemize}
                %     \item Thông số MQTT/HTTP/CoAP, lựa chọn kênh Internet (Wi-Fi, LTE, Ethernet).
            ...
            Sơ đồ khối phần cứng của hệ thống được xây dựng nhằm mô tả tổng thể kiến trúc và mối liên hệ giữa các khối chức năng chính, bao gồm khối xử lý trung tâm, các khối ngoại vi và hệ thống cấp nguồn. Mục tiêu của thiết kế sơ đồ khối là đảm bảo tính rõ ràng, dễ hiểu và khả năng mở rộng trong tương lai. 
                ...
                \paragraph{Sơ đồ khối hệ thống phần cứng:} 
                    ...
                    \begin{itemize}
                        ...
                        % Hệ thống được trang bị IC RTC PCF8563T/5,518, sử dụng nguồn dự phòng từ pin CR2032 (3.0V) nhằm duy trì liên tục thông tin thời gian khi nguồn cấp chính bị gián đoạn. Việc tích hợp RTC giúp hệ thống có khả năng thực hiện các tác vụ theo lịch trình và đồng bộ thời gian với các thiết bị khác trong mạng IoT.
                        ...
                        Hệ thống sử dụng IC TCA6416ARTWR để điều khiển tín hiệu Enable của các khối nguồn, đồng thời quản lý các tín hiệu reset, interrupt, GPIO của các khối ngoại vi và mô-đun mở rộng.

                        \item \textbf{Cổng PCIe mở rộng WAN/LAN:} 
                        % Hệ thống được thiết kế với 3 khe cắm PCIe, 1 khe dành cho mô-đun WAN và 2 khe dành cho mô-đun LAN. Các khe cắm này hỗ trợ kết nối linh hoạt với các mô-đun giao tiếp không dây (LoRa, Zigbee, BLE, Thread, NB-IoT,...) hoặc có dây (RS-485, CAN,...), giúp mở rộng khả năng kết nối của gateway.
                ...
                \paragraph{Sơ đồ khối nguồn:} \par
                    ...
                    Đối với các mô-đun mở rộng kết nối qua adapter, nguồn cấp mặc định là +3V3 hoặc +3V3\_CTRL. Trên mỗi adapter có tích hợp cơ chế điều khiển nguồn riêng cho từng mô-đun, đồng thời hỗ trợ chuyển đổi sang các mức điện áp khác khi cần thiết.
                    
                    Cách thiết kế này mang lại các ưu điểm sau:
                    \begin{itemize}
                        \item Cho phép chủ động bật/tắt nguồn từng mô-đun LAN/WAN tùy theo cấu hình và kịch bản sử dụng;
                ...
                \paragraph{Sơ đồ khối mạch adapter:}
                    % Mạch adapter được thiết kế nhằm chuẩn hóa giao diện kết nối giữa baseboard và các mô-đun mạng phổ biến trên thị trường. Adapter board được kết nối với baseboard thông qua đầu nối chuẩn vật lý PCI Express x1, trong đó các đường tín hiệu và nguồn được định nghĩa lại theo một chuẩn riêng thống nhất, hỗ trợ các giao thức cơ bản như USB, UART, SPI và I\textsuperscript{2}C.

                    % Trên mỗi adapter tích hợp một IC IO Expander, được điều khiển bởi MCU master hoặc slave trên baseboard (trong đó master quản lý các mô-đun WAN, slave quản lý các mô-đun LAN). IC này đảm nhiệm chức năng đóng/ngắt nguồn cấp cho mô-đun mạng, đồng thời đọc mã nhận dạng thiết bị (device ID) được mã hóa dưới dạng 4 bit thông qua 4 chân GPIO.
                    
                    % Dựa trên mã nhận dạng này, MCU trên baseboard có thể tự động xác định loại mô-đun được kết nối trong quá trình cấu hình gateway. Từ đó, hệ thống máy tính cấu hình sẽ truyền các tập lệnh giao tiếp phù hợp với từng loại mô-đun đến MCU quản lý, giúp đơn giản hóa quá trình tích hợp và tăng tính linh hoạt của hệ thống.
                    Mạch adapter chuẩn hóa kết nối giữa bo mạch chủ và các mô-đun mạng thông qua đầu nối vật lý PCI Express x1 với sơ đồ chân thống nhất cho các giao tiếp USB, UART, SPI và I²C. Sự tích hợp IC IO Expander trên mỗi adapter cho phép vi điều khiển chủ động quản lý nguồn và truy xuất mã định danh phần cứng (Device ID) 4-bit. Cơ chế này hỗ trợ cho việc tự động nhận diện mô-đun, giúp hệ thống tự cấu hình tập lệnh tương ứng, từ đó tối ưu hóa tính linh hoạt và đơn giản hóa quy trình tích hợp thiết bị.
    ...
    %             Khối xử lý trung tâm của hệ thống được thiết kế theo kiến trúc Dual-MCU, sử dụng hai mô-đun ESP32-S3-WROOM-1U-N16R8 đảm nhiệm vai trò Master và Slave.
    ...
    %             Dựa trên yêu cầu này, mạch RC được thiết kế với $R = 10\text{k}\Omega$ và $C = 1\mu\text{F}$. Thời gian để điện áp trên tụ đạt đến ngưỡng kích hoạt (xấp xỉ $0.75V_{CC} \approx 2.5V$) được xác định theo biểu thức:
    ...
    %                     \item \(V_{OL(\max)}\): mức điện áp logic thấp tối đa cho phép khi thiết bị nhận mức 0 (V)\footnote{Lấy theo datasheet của ESP32, xấp xỉ \(0.2V_{CC}\)\cite{Esp32S3WroomDatasheet}.}
    ...
    %             Dựa trên các khoảng giá trị này, điện trở kéo lên được lựa chọn là $3.3\text{k}\Omega$ cho bus phía master và $10\text{k}\Omega$ cho phía slave.
    ...
    %                 % Với kiến trúc Narrow VDC (NVDC), trong trường hợp không có pin, IC sẽ điều chỉnh để duy trì điện áp hệ thống ở mức tối thiểu cộng thêm một biên độ bù. Với giá trị $SYS\_MIN$ mặc định là 3.5V, điện áp VSYS điển hình được duy trì ở khoảng 3.65V.
    ...
    %                 % Khi đó, điện áp VSYS xấp xỉ bằng điện áp pin. Với pin LiPo sử dụng trong thiết kế, điện áp thay đổi từ 3.0V (hết pin) đến 4.2V (đầy pin). Tuy nhiên, để đảm bảo đủ mức điện áp tối thiểu cho các khối nguồn phía sau và tránh hiện tượng xả sâu, ngưỡng điện áp tối thiểu được thiết lập ở mức 3.5V. Do đó, VSYS dao động trong khoảng từ 3.5V đến 4.2V.
    ...
    %                 \begin{itemize}
    %                     \item Khi điện áp pin thấp (từ 3.0V đến dưới 3.5V): 
    %                     BATFET hoạt động ở chế độ tuyến tính (LDO mode) để duy trì điện áp hệ thống lớn hơn điện áp pin. VSYS được giữ ổn định tại mức $VSYS(MIN) + 150\,mV$, tương đương khoảng 3.65V.

    %                     \item Khi điện áp pin lớn hơn 3.5V: 
    %                     BATFET được bật hoàn toàn, điện áp VSYS sẽ bám theo điện áp pin với độ lệch khoảng 50\,mV, tức là $VSYS \approx VBAT + 50\,mV$. Do đó, VSYS tăng dần trong khoảng từ 3.55V đến 4.25V.

    %                     \item Trường hợp quá tải (chế độ Supplement): 
    %                     Khi dòng tải vượt khả năng cung cấp của VBUS, điện áp VSYS có xu hướng giảm. Khi VSYS thấp hơn điện áp pin, IC sẽ chuyển sang sử dụng năng lượng từ pin để bổ sung cho hệ thống. Khi đó, VSYS xấp xỉ bằng điện áp pin, nằm trong khoảng từ 3.5V đến 4.2V.
    %                 \end{itemize}
    %             \end{itemize}
    ...
    %             Tại đầu vào dương của bộ so sánh là một cầu chia áp nhằm tạo điện áp tham chiếu. Điện áp tham chiếu này có giá trị xấp xỉ 2.2V. Tại đầu vào âm của bộ so sánh là mạch RC delay, trong đó điện áp trên tụ tăng dần theo thời gian khi mạch được cấp nguồn. Khi điện áp trên tụ vượt qua mức tham chiếu 2.2V, đầu ra của bộ so sánh sẽ chuyển từ mức thấp sang mức cao, kích hoạt MOSFET kênh P đóng và cấp nguồn cho chân VCCI của IO Expander.
    ...
    %             \begin{itemize}
    %                 \item \textbf{Tại thời điểm $t = 0$}: mạch IO Expander vừa được cấp nguồn. Điện áp tại đầu vào âm của bộ so sánh bắt đầu tăng dần từ 0V lên mức 3.3V, trong khi điện áp tại đầu vào dương được giữ cố định ở mức 2.2V. Op-amp cho mức điện áp đầu ra cao (xấp xỉ 3.3V), khiến MOSFET kênh P ở trạng thái đóng, nguồn VCCI chưa được cấp.
    %                 \item \textbf{Tại thời điểm $t \approx 0.11\,ms$}: điện áp tại đầu vào âm vượt qua mức 2.2V, làm đầu ra op-amp bị kéo xuống mức thấp (xấp xỉ 0V). MOSFET kênh P chuyển sang trạng thái dẫn, cho phép nguồn VCCI được cấp cho IO Expander.
    %             \end{itemize}
    ...
    %             \begin{figure}[H]
    %                 \centering
    %                 \includegraphics[width=0.75\textwidth]{figures/sim_ioxdelayvcci.png}
    ...
    %             % IC sử dụng thạch anh ngoài với tần số 32.768\,kHz, được kết nối trực tiếp tới hai chân OSCI và OSCO. Thạch anh được lựa chọn với điện dung tải danh định khoảng 12.5\,pF và điện trở ESR nhỏ hơn 100k$\Omega$, đáp ứng các yêu cầu theo khuyến nghị của datasheet. Việc lựa chọn này giúp đảm bảo độ chính xác thời gian cao, tối ưu mức tiêu thụ năng lượng, đồng thời rút ngắn thời gian khởi động và nâng cao độ ổn định của hệ thống.
    %             Hệ thống tích hợp IC PCF8563 (I2C) cùng mạch Power OR-ing dùng diode Schottky nhằm đảm bảo dữ liệu thời gian thực được duy trì thông suốt trong mọi tình huống chuyển đổi nguồn. Việc bố trí diode TVS tại đường pin dự phòng là giải pháp cần thiết để triệt tiêu xung ESD, bảo vệ mạch khỏi rủi ro hỏng hóc khi thay pin. Việc sử dụng thạch anh 32.768\,kHz có ESR < 100k$\Omega$ và điện dung 12.5pF là sự tuân thủ yêu cầu của IC, giúp hệ thống đạt độ chính xác cao nhất và tối ưu hóa năng lượng vận hành.
    %             \begin{figure}[h]
    %                 \centering
    ...
    %             % Trong thiết kế, mạch sử dụng IC EMIF06-MSD02N16 được đặt ngay tại đầu vào của cổng kết nối thẻ Micro SD. IC này tích hợp sẵn các diode TVS để bảo vệ chống phóng tĩnh điện, các điện trở kéo lên với giá trị xấp xỉ 80--100k$\Omega$, cùng các điện trở giảm chấn mắc nối tiếp có giá trị khoảng 36--54$\Omega$ trên toàn bộ các đường tín hiệu SDIO của thẻ Micro SD. Việc sử dụng IC tích hợp giúp đơn giản hóa sơ đồ mạch, giảm số lượng linh kiện rời, đồng thời đảm bảo tuân thủ đầy đủ các khuyến nghị về bảo vệ và chất lượng tín hiệu trong quá trình thiết kế phần cứng.
    ...
    %             Để bảo đảm tính toàn vẹn dữ liệu khi giao tiếp với nhiều thiết bị ngoại vi qua dây dẫn dài, thiết kế tích hợp IC đệm TCA9803DGKR làm giải pháp phân tách bus chủ động. Lựa chọn này là kỹ thuật cần thiết để hệ thống vận hành ổn định ở chế độ Fast-mode (400\,kHz) mà không bị ảnh hưởng bởi giới hạn điện dung ký sinh 400pF. Việc sử dụng IC đệm giúp cô lập tải chỉ còn 10pF cho bus chính, đồng thời duy trì khả năng truyền dẫn tin cậy cho các nhánh mở rộng, giảm thiểu rủi ro lỗi bit do nhiễu và suy hao tín hiệu.
    %             \begin{figure}[H]
    %                 \centering
    ...
    %             Baseboard được thiết kế để kết nối với các mô-đun mạng WAN và LAN thông qua một Module Adapter trung gian. Module adapter được chuẩn hóa chân kết nối theo các giao thức phổ biến như UART, I\textsuperscript{2}C, SPI, USB cùng các chân GPIO hỗ trợ, nhằm đảm bảo khả năng tương thích linh hoạt giữa các mô-đun WAN/LAN và baseboard.
    ...
    %             Trong phiên bản hiện tại, Module Adapter WAN được triển khai thành hai mạch riêng biệt, bao gồm mạch adapter WAN LTE và mạch adapter WAN Ethernet. Adapter WAN LTE được thiết kế cho mô-đun SIM7600G-H-PCIE và adapter WAN Ethernet được phát triển dựa trên IC Ethernet PHY W5500. Mỗi adapter đều tích hợp các khối chức năng cần thiết để giao tiếp với baseboard, bao gồm khối cấp và điều khiển nguồn, bộ mở rộng ngõ vào/ngõ ra (IO expander), cùng các bus giao tiếp phần cứng tương ứng. 
    ...
    %             Đối với adapter WAN Ethernet, trên đường tín hiệu MDI giữa cổng RJ45 và IC Ethernet PHY cũng được bố trí các điện trở nối tiếp 33\(\Omega\) để giảm chấn, cùng các linh kiện thụ động khác theo khuyến nghị thiết kế trong tài liệu.\cite{Sim7600SeriesPcieHardwareDesign}
    ...
    %             Việc sử dụng adapter trung gian giúp tách biệt thiết kế baseboard khỏi các yêu cầu đặc thù của mô-đun WAN, qua đó nâng cao tính linh hoạt và khả năng mở rộng của hệ thống. 

    %             Đối với khối LAN, mạch adapter được triển khai dưới dạng ba adapter riêng biệt, mỗi adapter hỗ trợ một loại mô-đun mạng khác nhau, bao gồm:
    ...
    %             Trên mỗi adapter tích hợp một IC IO Expander, được sử dụng để điều khiển các tín hiệu IO của mô-đun, quản lý việc cấp nguồn và đọc mã nhận dạng thiết bị (device ID) trên adapter. Mỗi mô-đun được thiết kế với một adapter riêng biệt và được gán một mã thiết bị tương ứng nhằm phục vụ quá trình nhận diện tự động bởi MCU.

    %             Bảng mã nhận dạng thiết bị được quy định như sau:
                
    %             \begin{longtable}{|c|c|c|}
    %                 \caption{Device ID của các mô-đun WAN/LAN} \\
    %                 \hline
    ...
    %                         \item \textbf{Vùng Giao tiếp Modular (Interface Zone):} Bố trí dọc theo các cạnh bo mạch để thuận tiện cho thao tác cắm rút mô-đun.
    %                         \item \textbf{Vùng RF (RF Zone):} Dành riêng cho các mô-đun không dây và anten.
    %                     \end{itemize}
    %                 \end{itemize}
    %             % \begin{figure}[h]
    %             %     \centering
    ...
    \subsection{Firmware và Software}
        ...
        Phần mềm được xây dựng trên nền tảng FreeRTOS với cấu trúc phân lớp chuẩn hóa gồm Application, Middleware, BSP và Driver. Kiến trúc này tách hệ thống thành hai miền xử lý độc lập theo mô hình Dual-MCU Master--Slave, vừa tối ưu tài nguyên vừa đảm bảo tính mô-đun hóa cho các giao thức mở rộng.
        ...
        \begin{itemize}
            \item \textbf{Quản lý đa phương thức kết nối Internet:} Thiết lập và duy trì đường truyền qua Wi-Fi (Personal/Enterprise), LTE (giao thức USB) và Ethernet. Hệ thống tích hợp cơ chế tự động khôi phục kết nối (Auto-reconnect) và đồng bộ thời gian thực (SNTP/RTC) để đảm bảo tính thống nhất cho time-stamp dữ liệu.
        
            \item \textbf{Cơ chế Internet Fallback:} Hệ thống tự động chuyển đổi nguồn Internet dựa trên độ ưu tiên vật lý (Ethernet $>$ Wi-Fi $>$ LTE). Khi phát hiện mô-đun LTE hoặc Ethernet được kết nối, WAN-MCU chủ động kiểm tra trạng thái link để thay đổi kênh truyền ưu tiên, đồng thời tự động tái kết nối kênh dự phòng khi có sự cố, đảm bảo duy trì luồng dữ liệu Cloud liên tục.
        ...
        Trong kiến trúc ban đầu của gateway, loại mô-đun RF gắn vào từng khe phần cứng được coi là cố định tại thời điểm biên dịch. Toàn bộ thông tin đặc thù của mô-đun -- cú pháp lệnh AT, chuỗi điều khiển chân GPIO cho reset phần cứng, kiểu giao tiếp và cấu hình tương ứng -- được viết trực tiếp vào mã nguồn firmware. Điều này đặt ra một giới hạn căn bản: mỗi khi hệ thống cần chuyển sang loại mô-đun mới, bất kể lý do là thay thế phần cứng hay nâng cấp lên mô-đun có tính năng cao hơn, đội kỹ thuật phải sửa firmware, biên dịch lại và nạp lại cho toàn bộ thiết bị đã triển khai.

        Vấn đề này trở nên rõ ràng hơn khi tiến hành phân tích so sánh tám mô-đun BLE phổ biến trên thị trường. Phân tích cho thấy một điều nghịch lý: gần như hầu hết chức năng là giống nhau giữa tất cả các mô-đun -- đặt tên thiết bị, quét thiết bị lân cận, kết nối, ngắt kết nối, reset phần mềm, reset phần cứng -- nhưng cú pháp lệnh lại khác hoàn toàn. Ví dụ, chức năng đặt tên thiết bị được gọi bằng \texttt{AT+NAME=} trên HC-05, \texttt{AT+BLENAME=} trên mô-đun ESP32, và \texttt{AT+GAPDEVNAME=} trên nRF52. Việc chuẩn hóa theo lệnh là không khả thi; nhưng chuẩn hóa theo tên chức năng thì hoàn toàn có thể thực hiện, và đây chính là nền tảng của hệ thống Module Base Setting.

        \paragraph{Ý tưởng triển khai}

        Thay vì hard-code thông tin của từng loại mô-đun vào firmware, hệ thống Module Base Setting đưa toàn bộ kiến thức đặc thù của mô-đun ra ngoài firmware dưới dạng một file cấu hình JSON có thể cập nhật tại runtime. File JSON mô tả đầy đủ mọi điều firmware cần để vận hành mô-đun đó: loại giao tiếp vật lý và các tham số kèm theo, cùng danh sách tất cả chức năng mô-đun hỗ trợ. Mỗi chức năng trong danh sách được định nghĩa bởi một tên chuẩn hóa dùng làm khóa tra cứu, một template lệnh AT với placeholder cho tham số động, chuỗi điều khiển GPIO cần thực hiện trước và sau khi gửi lệnh, và giá trị timeout chờ phản hồi.
        
        Ví dụ khi sử dụng mô-đun cho BLE, bằng cách này, firmware không còn quan tâm đến việc mô-đun đang dùng là STM32WB55 hay nRF52840 -- nó chỉ biết rằng khi cần thực hiện chức năng \texttt{MODULE\_SCAN}, nó sẽ tra cứu tên này trong cấu hình đã nạp và thực thi theo chính xác những gì JSON mô tả. Toàn bộ logic đặc thù của mô-đun nằm trong file JSON, không nằm trong firmware. Thay đổi mô-đun đồng nghĩa với gửi một file JSON mới, không phải biên dịch lại firmware.
        
        Ba file cấu hình preset đã được chuẩn bị cho giai đoạn kiểm thử: Zigbee E180-ZG120B, BLE STM32WB55 và LoRa Wio-E5. Một file ánh xạ \texttt{stack\_id\_map.json} liên kết định danh khe phần cứng với preset mô-đun tương ứng.


        % \begin{figure}[H]
        %     \begin{minipage}[b]{0.55\textwidth}
        %         \paragraph{Luồng hoạt động}
        %         Hệ thống hoạt động theo hai giai đoạn tách biệt: giai đoạn nạp cấu hình mô-đun và giai đoạn thực thi lệnh điều khiển. Hình dưới đây mô tả toàn bộ luồng từ khi ứng dụng PC hoặc server gửi JSON config đến khi gateway sẵn sàng thực thi lệnh điều khiển mô-đun.
        %     \end{minipage}
        %     \hfill
        %     \begin{minipage}[t]{0.4\textwidth}
        %         \centering
