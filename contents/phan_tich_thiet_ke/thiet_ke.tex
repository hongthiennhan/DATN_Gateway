\section{Thiết kế}
    ...
    \subsection{Firmware và Software}
        ...
        \paragraph{Hệ thống cấu hình Module dựa trên JSON (Module Base Setting)}
        ...
        \begin{enumerate}
            ...
            Hệ thống được thiết kế theo kiến trúc Dual-MCU nhằm tối ưu hóa xử lý song song và tăng độ tin cậy. Luồng dữ liệu di chuyển qua một ``đường ống'' (\textit{Pipeline}) gồm các chặng chính:

            \begin{itemize}
                \item \textbf{Chặng 1 -- Air Interface:} Node ngoại vi gửi dữ liệu qua sóng vô tuyến như Zigbee, LoRa hoặc BLE.
            ...
            \item \textbf{Thông lượng dữ liệu và hiệu suất}

            Thông lượng ứng dụng thực tế $R_{app}$ là lượng dữ liệu hữu ích còn lại sau khi trừ overhead của giao thức tại tất cả các lớp. Hiệu suất băng thông được định nghĩa:

            \[
                \eta = \frac{R_{app}}{R_{PHY}}
            \]

            \textbf{Zigbee (E18-ZG120 / CC2530):} Tốc độ vật lý IEEE 802.15.4 đạt $250\,kbps$. Tuy nhiên, mỗi frame Zigbee chỉ chứa tối đa 99 byte payload ứng dụng sau khi trừ các header MAC/NWK/APS, tương đương 28 byte overhead trên tổng 127 byte. Khi truyền, thời gian hoàn thành một chu kỳ phát--nhận ACK khoảng $5\,ms$, tương đương thông lượng air xấp xỉ $158\,kbps$. Tuy nhiên, nút thắt nằm ở UART $115200\,bps$: module E18-ZG120 gửi frame HEX bất đồng bộ lên LAN-MCU cho mỗi gói nhận được, với kích thước khoảng 121 byte gồm 99 byte dữ liệu và 22 byte overhead ZCL/HEX frame.

            \textbf{LoRa (Wio-E5 / STM32WLE5JC) -- chế độ TEST/P2P:} Trong kịch bản kiểm thử, hệ thống sử dụng chế độ TEST/P2P (\texttt{AT+MODE=TEST}) thay vì LoRaWAN. Chế độ này bỏ qua duty cycle và stack LoRaWAN, cho phép phát liên tục, chỉ bị giới hạn bởi Time-on-Air. Tốc độ bit PHY tại SF7/125 kHz đạt $5.470\,bps$. Mỗi chu kỳ phát gồm thời gian truyền AT command qua UART khoảng $15\,ms$ và Time-on-Air khoảng $97.5\,ms$ cho gói 50 byte, tổng khoảng $115\,ms$, cho thông lượng ứng dụng xấp xỉ $3.5\,kbps$. Giới hạn payload tối đa qua AT command là 169 byte/gói do độ dài lệnh tối đa là 528 ký tự.

            \textbf{BLE (Native ESP32-S3):} BLE không qua module ngoài nên không có tầng UART. Thông lượng bị giới hạn bởi Connection Interval ($CI = 20\,ms$) và ATT MTU. Mặc định ATT MTU = 23 byte, tương ứng 20 byte dữ liệu, thông lượng chỉ khoảng $8\,kbps$. Khi kích hoạt Data Length Extension (DLE) và đàm phán MTU lên 247 byte, tương ứng 244 byte dữ liệu, thông lượng đạt khoảng $97.6\,kbps$.

            \begin{longtable}{|p{3.2cm}|p{2.3cm}|p{2.7cm}|p{1.4cm}|p{4cm}|}
                ...
                Zigbee & $250\,kbps$ & $\approx 80\,kbps$ & $32\%$ & UART $115200\,bps$ ($10.5\,ms/frame$) \\ \hline
            ...
            \begin{itemize}
                ...
                \item \textbf{Độ trễ UART $T_{UART}$:} khoảng $10.5\,ms/gói$ với Zigbee và $\approx 15\,ms$ với LoRa AT command.
                \item \textbf{Độ trễ gom gói $T_{batch}$:} cố định khoảng $50\,ms$ do cơ chế batch flush tại LAN-MCU.
                \item \textbf{Độ trễ SPI $T_{SPI}$:} truyền 1 KB mất khoảng $0.204\,ms$ tại xung nhịp $40\,MHz$.
            \end{itemize}
            ...
            \begin{itemize}
                ...
                \item \textbf{BLE:} Giao thức GATT giới hạn tối đa 8 kết nối đồng thời trên ESP32-S3 khi sử dụng NimBLE stack.
            \end{itemize}
