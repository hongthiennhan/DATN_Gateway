\section{Cơ sở lý thuyết}
    Nội dung trong phần này là nền tảng lý thuyết và kỹ thuật cốt lõi để định hướng và hiện thực hóa sản phẩm IoT Gateway trong đồ án. Thông qua việc nghiên cứu tổng quan về Internet vạn vật (IoT), vị trí, cấu trúc và nguyên lý hoạt động của gateway, cũng như các tiêu chuẩn thiết kế phần cứng chuyên sâu (như High-Speed PCB và Power Distribution Network - PDN), nội dung phần này cung cấp các luận cứ khoa học vững chắc.
    
    Qua đó, cơ sở lý thuyết giúp làm rõ các phương pháp nhằm giải quyết triệt để những vấn đề kỹ thuật then chốt đã được đặt ra trong \autoref{sec:cac_van_de}. Đồng thời, những kiến thức này là nền tảng giúp hiện thực hóa các yêu cầu được phân tích tại \autoref{sec:phan_tich_so_bo}, đặc biệt là việc thỏa mãn các ràng buộc thiết kế, đảm bảo tính module hóa, khả năng cấu hình linh hoạt cũng như đáp ứng trọn vẹn các yêu cầu chức năng và phi chức năng của hệ thống.
    \subsection{Tổng quan về Lĩnh vực}
        \subsubsection{Khái niệm Internet vạn vật (IoT)}
        ...
        \subsubsection{Vị trí và vai trò của IoT gateway}
    ...
    \subsection{Cơ sở lý thuyết về IoT Gateway}
        \subsubsection{Khái niệm IoT gateway}
        ...
        \subsubsection{Vị trí của IoT gateway trong kiến trúc IoT}
            ...
            \end{figure}
        \subsubsection{Các vai trò chính của IoT gateway}
            IoT Gateway không chỉ là một điểm kết nối đơn thuần mà là một hệ thống đa chức năng: tổng hợp dữ liệu từ nhiều thiết bị đầu cuối, chuyển đổi giao thức không đồng nhất, thực thi bảo mật, và xử lý dữ liệu tại biên. Các vai trò này phối hợp chặt chẽ để giúp IoT gateway trở thành thành phần trọng tâm của bất kỳ kiến trúc IoT hiệu quả.
                \paragraph{Tổng hợp và quản lý dữ liệu (data aggregation and management)}
                ...
                \paragraph{Chuyển đổi giao thức (protocol translation)}
                ...
                \paragraph{Khả năng điện toán biên (edge computing capabilities)}
                ...
                \paragraph{Thực thi bảo mật (security enforcement)}
                ...
                \paragraph{Quản lý kết nối và thiết bị (connectivity and device management)}
                ...
                \paragraph{Lưu trữ cục bộ và bộ nhớ đệm (local storage and caching)}
        ...
        \subsubsection{Cấu trúc của IoT gateway}
            ...
            \end{figure}
                ...
                \paragraph{Kiến trúc phần mềm}
                    ...
                    \textbf{Lớp trừu tượng phần cứng (Hardware Abstraction Layer -- HAL)}\\
                        HAL cung cấp tập API thống nhất để các module phần mềm tương tác với ngoại vi phần cứng (UART, SPI, I\textsuperscript{2}C, radio, \dots) mà không phụ thuộc vào chi tiết triển khai của từng nền tảng gateway. Điều này giúp tăng tính khả chuyển, cho phép cùng một mã nguồn ứng dụng có thể chạy trên các bo mạch phần cứng khác nhau chỉ bằng cách thay đổi HAL.
                    
                    \textbf{Lớp ngăn xếp cảm biến (Sensor Stacks)}\\
                        Lớp này hiện thực các driver và ngăn xếp giao thức cho mạng cảm biến và thiết bị như ZigBee, 6LoWPAN, EnOcean, BLE, Modbus, PROFIBUS,\dots{} Nó chịu trách nhiệm thu thập dữ liệu từ các nút cảm biến/chấp hành, chuyển về định dạng nội bộ thống nhất để các module quản lý dữ liệu và ứng dụng có thể sử dụng.
                    
                    \textbf{Lớp giao thức truyền thông (Protocols)}\\
                        Lớp giao thức cung cấp các protocol ở tầng ứng dụng phục vụ trao đổi dữ liệu với cloud hoặc hệ thống bên ngoài, điển hình như MQTT, CoAP, WebSockets, HTTP/HTTPS,\dots{} Các module này được xây dựng trên nền TCP/UDP của hệ điều hành và được các khối quản lý dữ liệu, quản lý cloud sử dụng để gửi/nhận các gói tin.
                    
                    \textbf{Lớp cập nhật firmware từ xa (FOTA/OTA)}\\
                        Module FOTA chịu trách nhiệm kiểm tra, tải về, xác thực và cài đặt các bản cập nhật phần mềm/firmware từ cloud. Ngoài việc đảm bảo gateway luôn được vá lỗi và cập nhật tính năng mới, lớp này còn hỗ trợ cơ chế rollback trong trường hợp cập nhật thất bại, giúp hệ thống duy trì trạng thái ổn định.
        ...
        \subsubsection{Nguyên lý hoạt động cơ bản của IoT Gateway}
            Quá trình hoạt động của gateway là một vòng lặp liên tục: thu thập dữ liệu từ thiết bị đầu cuối, xử lý và lọc tại chỗ, truyền gói tin lên cloud, nhận lệnh điều khiển từ cloud, và gửi lệnh xuống thiết bị. Trong suốt quá trình này, gateway phải đảm bảo tính nhất quán dữ liệu, bảo mật thông tin, phát hiện và xử lý lỗi, cũng như duy trì kết nối liên tục ngay cả trong điều kiện mạng không ổn định. Những yêu cầu này làm cho độ tin cậy, hiệu suất, và bảo mật trở thành những tiêu chí bắt buộc chứ không phải tuỳ chọn trong bất kỳ triển khai thực tế nào.
                \paragraph{Thu thập dữ liệu (Data Ingestion)}
                ...
                \paragraph{Xử lý cục bộ (Local Processing)}
                ...
                \paragraph{Định tuyến giao thức (Protocol Routing)}
                ...
                \paragraph{Điều khiển và quản lý từ xa (Remote Control and Management)}
                ...
                \paragraph{Dự phòng và lưu đệm (Failover and Caching)}
    ...
    \subsection{Phân tích các sản phẩm trên thị trường}
        ...
        \subsubsection{Các tính năng quan trọng của gateway trên thị trường}
            ...
            \end{itemize}
        % \subsubsection{Ví dụ về các gateway thực tế trên thị trường}
        %     \paragraph{M5Stack IoT gateway series}
        ...
        %             \end{itemize}
        %     \paragraph{RAK7391 WisGate Connect}
        ...
        %     \paragraph{SRT-IMX8P industrial gateway}
        ...
        \subsection{Cơ sở lý thuyết về Thiết kế phần cứng}
            \subsubsection{Thiết kế High-Speed PCB}
                ...
                \paragraph{Tính toàn vẹn tín hiệu (Signal Integrity) và đường hồi dòng (Return Path)}
                ...
                \paragraph{Quy tắc bố trí để giảm nhiễu, crosstalk và cải thiện EMI}
            ...
            \subsubsection{Power Distribution Network – PDN}
                ...
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.75\textwidth]{figures/lythuyet_pdn.png}
