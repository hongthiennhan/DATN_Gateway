\chapter{Cơ sở Lý thuyết}
    ...
    \section{Cơ sở lý thuyết về IoT Gateway}
        ...
        \subsection{Nguyên lý họat động cơ bản của IoT Gateway}
        ...
        \section{Cơ sở lý thuyết về Thiết kế phần cứng}
            ...
            \subsubsection{Tính toàn vẹn tín hiệu (Signal Integrity) và đường hồi dòng (Return Path)}
                \paragraph{Signal Integrity (SI)}
                    Signal Integrity là mức độ mà tín hiệu đến đầu nhận vẫn giữ được biên độ, dạng xung và thời điểm chuyển mức như dự kiến. Mất toàn vẹn tín hiệu thể hiện qua méo dạng dạng sóng, overshoot/undershoot, rung (ringing), jitter và thậm chí lỗi dữ liệu.
                    ...
                    \begin{itemize}
                        ...
                        \item Bố trí return path không liên tục, tạo vòng dòng lớn và tăng cảm kháng.
                        \item Đặc tính vật liệu (tổn hao điện môi, hiệu ứng da, tán sắc) trên đường dài.
                    \end{itemize}
                    
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.5\textwidth]{figures/lythuyet_ringing.png}
                ...
                \paragraph{Return path và vai trò của mặt phẳng tham chiếu}
                    Mọi dòng tín hiệu high-speed đều đi kèm một dòng hồi tương ứng, thường chạy trên mặt phẳng tham chiếu (ground hoặc power plane) ngay sát lớp tín hiệu. Trong miền tần số cao, dòng hồi có xu hướng đi theo đường ngắn nhất và song song ngay dưới trace để giảm cảm kháng vòng lặp; nếu mặt phẳng bị xẻ rãnh hay có khe hở, return path bị cưỡng bức vòng xa, làm tăng cảm kháng, crosstalk và EMI.
                    
                    \begin{itemize}
                        \item Luôn định tuyến tín hiệu tốc độ cao trên lớp kề sát một mặt phẳng tham chiếu liên tục.
                        \item Tránh chạy trace băng qua khe hở trên plane; nếu buộc phải đổi lớp, cần cung cấp via mass (stitching/return via) để dòng hồi chuyển lớp liên tục.
                    \end{itemize}
                    
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.5\textwidth]{figures/lythuyet_highspeedlayout.png}
                    ...
                    Khi tín hiệu lan truyền trên đường truyền có trở kháng đặc trưng \(Z_0\) và gặp một điểm có trở kháng tải \(Z_L\) khác, một phần năng lượng bị phản xạ ngược về nguồn, gây méo dạng sóng và có thể tạo rung. Hệ số phản xạ được cho bởi:
                    \[
                    \Gamma = \frac{Z_L - Z_0}{Z_L + Z_0}
                    \]
                    
                    Giá trị \(\Gamma\) càng xa 0, biên độ sóng phản xạ càng lớn. Điều này là lý do cốt lõi phải kiểm soát và khớp trở kháng trong thiết kế high-speed.
                    
                    Trong các hệ thống số tốc độ cao hiện nay, hầu hết các linh kiện giao tiếp (PHY Ethernet, USB transceiver, HDMI/DisplayPort, DDR PHY, transceiver MIPI, ADC/DAC tốc độ cao, v.v.) đều được thiết kế với trở kháng vào/ra danh định phù hợp với các giá trị trở kháng chuẩn trên PCB và cáp truyền dẫn. Các chuẩn phổ biến gồm 50~\(\Omega\) single-ended cho nhiều đường RF và high-speed nói chung, 90~\(\Omega\) vi sai cho các giao thức như USB, HDMI, MIPI, và 100~\(\Omega\) vi sai cho Ethernet hoặc nhiều giao thức differential khác.

                    Khi đường mạch trên PCB không được thiết kế để có trở kháng đặc trưng khớp với giá trị mà linh kiện và chuẩn giao thức yêu cầu, sẽ xuất hiện không khớp trở kháng tại điểm nối giữa chip--PCB--cáp. Điều này làm hệ số phản xạ \(\Gamma\) tăng lên, gây overshoot, ringing, méo dạng biên và có thể làm giảm biên độ hoặc sai lệch timing tại đầu nhận. Vì lý do đó, các đường truyền high-speed luôn được thiết kế \textit{controlled impedance} bám theo các giá trị chuẩn (50~\(\Omega\), 90~\(\Omega\), 100~\(\Omega\), \ldots) để tận dụng tối đa đặc tính trở kháng chuẩn hóa mà các nhà sản xuất linh kiện đã định nghĩa sẵn.
                
            \subsubsection{Kiểm soát trở kháng}
            
                \paragraph{Trở kháng đặc trưng và các chuẩn thông dụng.}
                
                    Trở kháng đặc trưng của đường truyền trên PCB phụ thuộc vào hình học trace và hằng số điện môi của lớp cách điện. Một số giá trị điển hình:
                    \begin{itemize}
                        \item Single-ended: 50~\(\Omega\) trong phần lớn ứng dụng high-speed và RF.
                        \item Differential pairs: khoảng 90~\(\Omega\) cho USB, HDMI; khoảng 100~\(\Omega\) cho Ethernet; khoảng 120~\(\Omega\) cho CAN bus, \ldots
                    \end{itemize}
                    
                    Như vậy, mỗi chuẩn giao thức thường có dải trở kháng đặc trưng riêng, nhằm đảm bảo tương thích và toàn vẹn tín hiệu.
                    
                    % Hình gợi ý 5: bảng nhỏ hoặc sơ đồ minh hoạ đường single-ended 50~Ω và cặp vi phân 100~Ω với kích thước W, S, H.
                    
                \paragraph{Các yếu tố quyết định trở kháng của trace.}
                
                    Trở kháng \(Z_0\) của một microstrip/stripline trên PCB chịu ảnh hưởng bởi:
                    \begin{itemize}
                        \item Hằng số điện môi \(\varepsilon_r\) và loss tangent của vật liệu nền.
                        \item Độ dày lớp điện môi giữa trace và mặt phẳng tham chiếu.
                        \item Chiều rộng trace và độ dày đồng.
                        \item Độ nhám bề mặt đồng, đặc biệt ở tần số cao.
                        \item Với cặp vi sai: khoảng cách giữa hai trace (spacing) quyết định trở kháng vi sai \(Z_{\mathrm{diff}}\).
                    \end{itemize}
                    ...
                    % Hình gợi ý 6: cắt lớp (cross-section) microstrip/stripline thể hiện W, H, T, Er; thêm version cặp vi phân.
                    
                \paragraph{Lý do phải kiểm soát và khớp trở kháng.}
                
                    Kiểm soát trở kháng nghĩa là đảm bảo đường mạch có \(Z_0\) gần với giá trị mục tiêu (ví dụ 50~\(\Omega\), 90~\(\Omega\), 100~\(\Omega\)) trong toàn bộ chiều dài, nhằm:
                    \begin{itemize}
                        \item Giảm phản xạ, tránh overshoot, ringing, méo dạng biên.
                        \item Tối ưu truyền năng lượng từ nguồn đến tải.
                        \item Đáp ứng yêu cầu của chuẩn giao thức để đảm bảo tương thích.
                    \end{itemize}
            ...
            \subsubsection{Quy tắc bố trí để giảm nhiễu, crosstalk và cải thiện EMI}
                ...
                \paragraph{Giảm nhiễu xuyên kênh (crosstalk).}
                    ...
                    Crosstalk là nhiễu do ghép điện dung và cảm ứng giữa các đường tín hiệu gần nhau, gây méo dạng hoặc lỗi trên đường nạn nhân. Các biện pháp điển hình:
                    \begin{itemize}
                        \item Tăng khoảng cách giữa các trace, đặc biệt giữa các tín hiệu tốc độ cao khác nhau; một quy tắc kinh nghiệm là khoảng cách \(\ge 3\) lần chiều cao tới plane.
                        \item Giữ cặp vi phân chạy sát nhau, song song, với khoảng cách không đổi, để tăng ghép vi sai và giảm nhiễu đồng mode.
                        \item Tránh chạy song song dài giữa tín hiệu nhạy cảm và tín hiệu nhiễu cao; nếu cần băng qua thì ưu tiên vuông góc giữa các lớp.
                        \item Sử dụng mặt phẳng tham chiếu liên tục và return path ngắn để giảm ghép cảm ứng.
                    \end{itemize}
                    
                    % Hình gợi ý 7: minh hoạ hai trace song song quá gần gây crosstalk so với trường hợp tách xa hơn; thêm hình cặp vi phân chạy sát nhau.
                    
                \paragraph{Cải thiện EMI/EMC.}
                    
                    EMI là bức xạ hoặc dẫn nhiễu điện từ ra/vào hệ thống, thường tăng mạnh khi layout không tốt. Một số quy tắc high-speed:
                    \begin{itemize}
                        \item Giữ diện tích vòng dòng (loop area) nhỏ bằng cách đặt trace ngay trên plane liên tục, dùng via mass khi đổi lớp.
                        \item Tránh chạy trace high-speed dọc theo biên PCB dài không che chắn; khi cần giảm bức xạ, ưu tiên route trong lớp trong (stripline).
                        \item Hạn chế via không cần thiết trên đường high-speed; với PCB dày có thể dùng backdrill để giảm stub.
                        \item Phân vùng rõ miền digital, analog, RF; bố trí decoupling và lọc gần chân IC nhạy cảm.
                    \end{itemize}
                    
                    % Hình gợi ý 8: hình vòng dòng lớn vs vòng dòng nhỏ (trace xa plane vs gần plane), minh hoạ trường bức xạ.
                    
                \paragraph{Một số nguyên tắc layout \& routing high-speed tiêu biểu.}
                
                    \begin{itemize}
                        \item Thiết kế stackup ngay từ đầu, với lớp tín hiệu xen kẽ lớp plane và xác định trước các lớp dùng cho tín hiệu high-speed.
                        \item Áp dụng quy tắc độ dài (length matching) cho cặp vi phân và các bus song song để giảm lệch thời gian (skew).
                        \item Tuân thủ chặt chẽ chiều rộng, khoảng cách, lớp và via theo constraint của từng giao thức.
                        \item Tránh góc gãy 90 độ trên đường high-speed; ưu tiên hai góc 45 độ hoặc bo cong để giảm gián đoạn trở kháng.
                    \end{itemize}
                    
                    % Hình gợi ý 9: ví dụ layout sai (trace đi vòng, băng qua khe plane, nhiều góc 90 độ) và layout đúng (đường ngắn, trên plane liên tục, góc 45 độ).
