...
void mqtt_update_received_data(const unsigned char *data, uint16_t data_len)
{
    ...
    size_t copy_len = data_len < sizeof(mqtt_last_received_data) - 1 ? data_len : sizeof(mqtt_last_received_data) - 1;
...
static void mqtt_event_handler(struct mg_connection *c, int ev, void *ev_data)
{
    ...
    case MG_EV_MQTT_MSG:
        ...
        if (mm->data.len > 0)
        {
            mqtt_update_received_data((const unsigned char *)mm->data.buf, mm->data.len);
        }
...
static void build_telemetry_payload(char *payload, size_t payload_size, time_t timestamp)
{
    ...
    if (strlen(mqtt_last_received_data) > 0)
    {
        ...
        escape_json_string(mqtt_last_received_data, escaped_data, sizeof(escaped_data));
        snprintf(temp_buffer, 2048, ",\"last_received_data\":\"%s\"", escaped_data);
        strncat(payload, temp_buffer, payload_size - strlen(payload) - 1);

        // Add hex representation
        char *hex_data = data_to_hex_string((const unsigned char *)mqtt_last_received_data,
        ...
        // Add metadata
        snprintf(temp_buffer, 2048, ",\"last_received_time\":%ld", mqtt_last_received_time);
        strncat(payload, temp_buffer, payload_size - strlen(payload) - 1);

        snprintf(temp_buffer, 2048, ",\"total_messages_received\":%d", mqtt_total_messages_received);
        strncat(payload, temp_buffer, payload_size - strlen(payload) - 1);
    }
    pthread_mutex_unlock(&mqtt_data_mutex);
