...
static int store_pending_message(uint32_t msg_id, const char *data, size_t len)
{
    ...
    for (int i = 0; i < MAX_PENDING_MESSAGES; i++)
    {
        if (pending_messages[i].message_id == 0)
        {
            ...
            size_t copy_len = len < sizeof(pending_messages[i].payload) ? len : sizeof(pending_messages[i].payload) - 1;
...
static void check_message_timeouts(void)
{
    ...
    for (int i = 0; i < MAX_PENDING_MESSAGES; i++)
    {
        if (pending_messages[i].message_id != 0 && !pending_messages[i].ack_received)
        {
            ...
            if (elapsed * 1000 >= ACK_TIMEOUT_MS)
            {
                if (pending_messages[i].retry_count < MAX_RETRY_COUNT)
                {
                    ...
                    {
                        mg_send(tcp_connection, pending_messages[i].payload, pending_messages[i].payload_len);
                        pending_messages[i].send_time = current_time;
                        pending_messages[i].retry_count++;

#ifdef DEBUG
                        printf("TCP: Retrying message %u (attempt %d)\n",
                               pending_messages[i].message_id, pending_messages[i].retry_count + 1);
#endif
                    }
                }
                else
...
static void send_ack_response(uint32_t msg_id)
{
    ...
    snprintf(ack_msg, sizeof(ack_msg),
             "{\"type\":\"ack\",\"message_id\":%u,\"client_id\":\"%s\",\"timestamp\":%ld}%s",
             msg_id, config->client_id, time(NULL), config->protocol_settings.message_delimiter);
...
static int tcp_send_data_with_ack(const char *data, size_t len)
{
    ...
    if (data[0] == '{')
    {
        snprintf(json_with_id, len + 256, "{\"message_id\":%u,%s", msg_id, data + 1);
    }
    else
    {
        snprintf(json_with_id, len + 256, "{\"message_id\":%u,\"data\":\"%s\"}", msg_id, data);
    }
    ...
    memcpy(send_buffer + new_len, config->protocol_settings.message_delimiter, delimiter_len);
...
static void tcp_process_received_data(const char *data, size_t len)
{
    ...
    else
    {
        ...
        if (id_start)
        {
            uint32_t received_msg_id = (uint32_t)atol(id_start + strlen("\"message_id\":"));
